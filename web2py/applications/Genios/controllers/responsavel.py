...
def index():
    ...
    links = [lambda row:  A('Editar',_href=URL("responsavel","alterar_aula",args=[row]) ),
...
'''    professores = db( db.tb_aula.id_aula == request.args(0) and
       (db.tb_aula.hora_inicial >=  db.tb_horario_livre.hora_inicial & db.tb_aula.hora_final <= db.tb_horario_livre.hora_final )
       ).select(db.tb_aula.hora_inicial, db.tb_aula.hora_final)
'''
#    return dict(professores = profesores)
...
def criar_aula():
    ...
    form.vars.fk_id_cidade = session.cidadeUsuario
